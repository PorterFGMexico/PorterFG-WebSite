	...
	getResponse: function() {
		...
		if (response.errorThrown) {
			...
			if (xml && xml.documentElement.localName === 'error' && xml.documentElement.namespaceURI === 'DAV:') {
				var messages = xml.getElementsByTagNameNS('https://sabredav.org/ns', 'message');
				var exceptions = xml.getElementsByTagNameNS('https://sabredav.org/ns', 'exception');
				if (messages.length) {
...
OC.Uploader.prototype = _.extend({
	 ...
	 * also see article @link https://blog.new-bamboo.co.uk/2012/01/10/ridiculously-simple-ajax-uploads-with-formdata
	 */
		...
		if ($uploadEl.exists()) {
			...
			this.fileUploadParam = {
				...
				add: function(e, data) {
					...
					// https://stackoverflow.com/a/20448357
